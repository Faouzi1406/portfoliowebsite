import { c as create_ssr_component, h as compute_rest_props, f as each, v as validate_component, m as missing_component, e as escape, g as getContext, d as add_attribute, i as createEventDispatcher, a as setContext } from "../../../../chunks/index2.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `${$$result.head += `<!-- HEAD_svelte-y548t_START -->${$$result.title = `<title>
    ${escape(projects.projectName.split("/")[1])} - ${escape(projects.projectName.split("/")[0])}
  </title>`, ""}<!-- HEAD_svelte-y548t_END -->`, ""}
...
<div class="${"relative"}">${user.role == 1 ? `<a class="${"border h-fit py-2 px-4 btn-outline rounded-md top-5 w-fit absolute left-5"}"${add_attribute("href", `/projects/edit/${projects.id}`, 0)}>Edit
    </a>` : ``}</div>

<div class="${"flex justify-center h-screen"}"><article class="${"w-full md:w-3/3 border-x"}"><div class="${"relative"}"><img${add_attribute("src", `../${projects.projectThumb.split("/")[2]}`, 0)} class="${"w-full h-[500px] object-cover"}" alt="${"Thumbmail"}">

      <div class="${"absolute bottom-0 text-white w-full backdrop-blur-xl bg-black/30 p-2 flex"}"><h1 class="${"text-3xl first-letter:capitalize"}">${escape(projects.projectName.split("/")[1])}</h1>
        <div class="${"absolute right-0 flex items-center gap-2"}"><img${add_attribute("src", projects.projectAvatar, 0)} class="${"w-10 rounded-full border"}" alt="${"profiel foto"}">
          <p class="${"mr-5 first-letter:capitalize"}">${escape(projects.projectName.split("/")[0])}</p></div></div></div>

    <div class="${"py-2 px-2 prose max-w-full text-white"}">${validate_component(SvelteMarkdown, "SvelteMarkdown").$$render($$result, { source }, {}, {})}</div></article></div>`;
});
export {
  Page as default
};
